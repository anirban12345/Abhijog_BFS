...
class Complainant_Controller extends MY_Controller 
{
	
	public function __construct() 
	{
		parent::__construct();	
		$complainant_userdtls=$this->session->userdata('complainant_userdtls');

		//echo '<pre>';
		//print_r(unserialize($userdtls[0]->ug_permission));
		
		if(!empty($complainant_userdtls))
		{
			if(!$complainant_userdtls[0]->logged_in) {			
				redirect('ComplainantLogin', 'refresh');
			}
			else
			{
	...
	public function render_template($page = null, $data = array())
	{
		...
		$this->Globalmodel->savedata('activity_log',$acitvity);						
		*/
		$this->load->view('complainant/header',$this->data);		
		$this->load->view('complainant/sidemenu',$this->data);
		$this->load->view($page, $this->data);
		$this->load->view('complainant/footer',$this->data);
	}
